 ...
 * Negative values can also be used. We do not use 0.5 for noon calculation. 
...
class com.fusioncharts.helper.FCDateTime {
	...
	private static var currentMonth:Number = FCDateTime.currentDate.getUTCMonth();
	...
	*/
	function FCDateTime() {
		...
		if (arguments.length == 1) {
			...
			//TODO: Do we need to update dateGiven and timeGiven flags?
		}
	...
	*/
	public static function setDateFormat(strFormat:String):Void {
	...
	*/
	private function buildDateTimeStr(y:Number, m:Number, d:Number, h:Number, mn:Number, s:Number):String {
		var dateStr:String = "";
		var timeStr:String = "";
		//TODO: Add left 0 padding to the values		
		if (dateGiven) {
			//Based on date format, build the date string
			switch (FCDateTime.dateFormat) {
			case "dmy" :
				dateStr = d+"/"+m+"/"+y;
				break;
		...
		if (timeGiven) {
			timeStr = (dateGiven) ? " " : "";
			timeStr = timeStr+padTime(h)+":"+padTime(mn)+":"+padTime(s);
		}
		return dateStr+timeStr;
	}
	/**
	 * padTime method adds padding to given time value.
	 *	@param	timeToken	Time token to add padding to
	 *	@return				Token with padding added to it.
	*/
	private function padTime(timeToken:Number):String {
		var pTimeToken:String = (timeToken<10) ? ("0"+String(timeToken)) : String(timeToken);
		return pTimeToken;
	}
	/**
	...
	*/
	public function dayName():String {
		...
		var dayNames:Array = new Array("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday");
		//Subtract index by 1 as array is 0 index based
		dayIndex--;
		if (useShortName) {
			return (dayNames[dayIndex].substring(0, 3));
		} else {
