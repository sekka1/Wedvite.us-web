...
* InteractiveLegend class helps us generate legend for a chart.
*/
...
class com.fusioncharts.helper.InteractiveLegend {
	...
	//If COLUMNS in chart be clustered 
	...
	*	@param	clustered		if COLUMNs be clustered in chart
	*/
	function InteractiveLegend (targetMC:MovieClip, fontProp:Object, clickMode:Boolean, alignPos:String, xPos:Number, yPos:Number, maxWidth:Number, maxHeight:Number, allowDrag:Boolean, stageWidth:Number, stageHeight:Number, bgColor:String, bgAlpha:Number, borderColor:String, borderThickness:Number, borderAlpha:Number, scrollBgColor:String, scrollBarColor:String, btnColor:String, clustered:Boolean){
	...
	*						are click-able.
	*	@param	active		active status of the legend item
	*	@param	chartType	chart type
	*/
	public function addItem (itemName:String, itemColor:String, itemId:Number, active:Boolean, chartType:String){
	...
	/**
	 * clearItems method clears all items from
	 * the array holding data for legend.
	 */
	...
	*/
	private function getLegendStr():String{
		...
		var currentArrayId:Number = -1
		var currentChartType:String = ''
		...
		for (i=count-1; i>=0; i--){
			...
			if(!this.clustered){
				...
				strText = strText + ((this.clickMode==true)?("<A HREF='asfunction:localFN," + itemId + "'>"):("")) +  marker + "<font face='" + this.fontProp.font + "' color='#" + this.fontProp.color + "' size='" + this.fontProp.size + "'>"+ this.items[i].name + "</font>" + ((this.clickMode==true)?("</A>"):(""));
				//Add spacers - If legend is in vertical alignment (alignPos = right), we add line break
				strText = strText + ((this.alignPos == "RIGHT")?("<BR>"):("      "));
			} else{
				if(currentChartType != this.items[i].chartType){
					currentChartType = this.items[i].chartType
					currentArrayId++
				}
				if(currentChartType == 'COLUMN'){
					//Add the legend item name - with/without link
					strText = ((this.clickMode==true)?("<A HREF='asfunction:localFN," + itemId + "'>"):("")) +  marker + "<font face='" + this.fontProp.font + "' color='#" + this.fontProp.color + "' size='" + this.fontProp.size + "'>"+ this.items[i].name + "</font>" + ((this.clickMode==true)?("</A>"):(""));
					//Add spacers - If legend is in vertical alignment (alignPos = right), we add line break
					strText = strText + ((this.alignPos == "RIGHT")?("<BR>"):("      "));
					//
					arrStrings[currentArrayId] = strText + arrStrings[currentArrayId]
				}else{
					//Add the legend item name - with/without link
					arrStrings[currentArrayId] += ((this.clickMode==true)?("<A HREF='asfunction:localFN," + itemId + "'>"):("")) +  marker + "<font face='" + this.fontProp.font + "' color='#" + this.fontProp.color + "' size='" + this.fontProp.size + "'>"+ this.items[i].name + "</font>" + ((this.clickMode==true)?("</A>"):(""));
					//Add spacers - If legend is in vertical alignment (alignPos = right), we add line break
					arrStrings[currentArrayId] += ((this.alignPos == "RIGHT")?("<BR>"):("      "));
				}
		...
		// for clustered data
		if(this.clustered){
			// concat strings to get the final string
			// variable reinitialised before concatenation
			strText = ''
			for(var i=0;i<arrStrings.length;++i){
				strText += arrStrings[i]
			}
	...
	*/
	...
	/*
	* destroy function cleans up the given instance of InteractiveLegend class.
	*/
