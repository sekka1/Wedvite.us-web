...
*
* Column3DChart chart extends the SingleYAxis3DChart class to render a
* 3D Column Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.Column3DChart extends SingleYAxis3DChart 
{
	...
	*/
	function Column3DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Single Series 3D Column Chart", Logger.LEVEL.INFO);
	...
	*/
	private function returnDataAsObject (label : String, value : Number, color : String, alpha : Number, toolText : String, link : String, showLabel : Number, showValue : Number) : Object 
	{
		//Create a container
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "SET")
					{
						...
						this.data [this.num] = this.returnDataAsObject (setName, setValue, setColor, setAlpha, setToolText, setLink, setShowLabel, setShowValue);
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		this.params.plotBorderColor = formatColor (getFV (atts ["plotbordercolor"] , "333333"));
	...
	*/
	private function calculatePoints ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.num > 2)
			{
				this.data [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (colSpace * (i - 1)) + ((i - 0.5) * colWidth) - this.config.shifts.xShift;
			} else 
			{
				this.data [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (colSpace * i) + ((i - 0.5) * colWidth) - this.config.shifts.xShift;
			}
			//Set the y position
			this.data [i].y = this.config.basePlanePos + this.config.shifts.yShift;
			//Height for each column
			dataEndY = this.getAxisPosition (this.data [i].value, this.config.yMax, this.config.yMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
			//Negate to cancel Flash's reverse Y Co-ordinate system
			this.data [i].h = - (dataEndY - this.config.basePlanePos);
			//Width
			this.data [i].w = colWidth;
			//Store value textbox y position
			this.data [i].valTBY = dataEndY
		}
	}
	/**
	...
	private function drawColumns ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.data [i].isDefined)
			{
				...
				colIns = new Column3D (colMC, this.data [i].x, this.data [i].y, this.config.shifts.wFront, this.data [i].h, this.config.shifts.xShift, this.config.shifts.yShift, this.data [i].color, this.params.showPlotBorder, this.params.plotBorderColor, this.params.plotBorderAlpha);
				//Draw the column
				...
				if (this.params.animation)
				{
					...
					if (Math.abs (this.data [i].h) > (this.config.shifts.yShift * 1.5))
					{
						animY = 100;
					} 
					else
					...
					this.styleM.applyAnimation (colMC, this.objects.DATAPLOT, this.macro, this.data [i].x, 0, this.data [i].y, 0, 100, 100, animY, null);
				}
				//Apply filters
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.data [i].isDefined && this.data [i].showValue)
			{
				...
				if (this.params.placeValuesInside)
				{
					...
					yPos = (this.data [i].h >= 0) ? (this.data [i].valTBY + this.params.valuePadding + this.config.shifts.yShift) : (this.data [i].valTBY - this.params.valuePadding + this.config.shifts.yShift);
				}else
				{
					vAlign = (this.data [i].h >= 0) ?"top" : "bottom";
					yPos = (this.data [i].h >= 0) ? (this.data [i].valTBY - this.params.valuePadding) : (this.data [i].valTBY + this.params.valuePadding + this.config.shifts.yShift);
				}
				if (this.params.placeValuesInside)
				{
					xPos = this.data [i].x;
				...
				{
					xPos = (this.data [i].value < 0) ? (this.data [i].x) : (this.data [i].x + this.config.shifts.xShift);
				}
				//Align position
				align = "center";
				...
				if (this.params.placeValuesInside && ((this.params.valuePadding + valueObj.height) >= Math.abs (this.data [i].h)))
				{
					...
					if ( ! this.params.rotateValues)
					{
						...
						{
							//Show above
							valueObj.tf._y = this.data [i].y - this.config.shifts.yShift - this.data [i].h - valueObj.height - this.params.valuePadding;
					...
					else
					{
						...
						{
							//Show above
							valueObj.tf._y = this.data [i].y - this.data [i].h - this.params.valuePadding - this.config.shifts.yShift;
				...
				if (( ! this.params.placeValuesInside) && (this.data [i].h >= 0) && ((yPos - valueObj.height) <= this.elements.canvasBg.y))
				{
				...
				if (( ! this.params.placeValuesInside) && (this.data [i].h < 0) && ((yPos + valueObj.height) >= this.elements.canvasBg.toY))
				{
