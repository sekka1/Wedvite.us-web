 /**
* @class StackedBar2DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* StackedBar2DChart chart extends the SingleYAxis2DHorizontalChart class to render a
...
class com.fusioncharts.core.charts.StackedBar2DChart extends SingleYAxis2DHorizontalChart 
{
	...
	*/
	function StackedBar2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Stacked 2D Bar Chart", Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "VGRID", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "DATAPLOT", "TOOLTIP", "VLINES", "LEGEND");
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		this.params.chartRightMargin = getFN (atts ["chartrightmargin"] , 25);
	...
	*/
	private function calculatePoints ()
	{
		...
		if (this.params.yAxisName != "")
		{
			...
			//Indicator for legend height - will be used later to place yAxis Label
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			this.categories [i].y = this.elements.canvas.y + (interBarSpace * i) + barHeight * (i - 0.5);
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].data [i].isDefined)
				{
					...
					//Set the x position
					this.dataset [j].data [i].x = (this.dataset [j].data [i].w >= 0) ?posX : negX;
					//Height
					...
					this.dataset [j].data [i].valTBX = this.config.basePlanePos + this.dataset [j].data [i].x + (this.dataset [j].data [i].w / 2);
				}
			}
	...
	*/
	private function drawBars ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			stackMC = this.cMC.createEmptyMovieClip ("StackBar_" + i, depth);
			//Set position
			stackMC._x = this.config.basePlanePos;
			stackMC._y = this.categories [i].y;
			for (j = 1; j <= this.numDS; j ++)
			{
				//If defined
				if (this.dataset [j].data [i].isDefined)
				{
					...
					barMC = stackMC.createEmptyMovieClip ("Bar_" + j, j);
					if (this.params.useRoundEdges){
						var barIns:RoundBar2D = new RoundBar2D(barMC, this.dataset [j].data [i].w, this.dataset [j].data [i].h, this.dataset [j].data [i].cornerRadius, this.dataset [j].data [i].color[0].toString(16), this.params.plotBorderThickness, this.params.plotBorderAlpha);
					}else{
						//Create bar instance
						var barIns : Bar2D =  new Bar2D (barMC, this.dataset [j].data [i].w, this.dataset [j].data [i].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.dataset [j].data [i].color, this.dataset [j].data [i].alpha, this.dataset [j].data [i].ratio, this.params.plotFillAngle, false, this.dataset [j].data [i].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
					}
					//Draw the bar
					barIns.draw ();
					//Set x position
					barMC._x = this.dataset [j].data [i].x;
					//Set the alpha of entire bar as the first alpha in list of alpha
					barMC._alpha = this.dataset [j].data [i].alpha [0];
		...
		if (this.params.showSum)
		{
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if (this.sums [i].sum != 0)
				{
					...
					xPos = this.config.basePlanePos + ((this.sums [i].sum < 0) ?this.sums [i].nX : this.sums [i].pX);
					//v-Align position
					vAlign = "middle";
					//Convey alignment to rendering object
					valueStyleObj.align = align;
					valueStyleObj.vAlign = vAlign;
					angle = 0;
					valueObj = createText (false, this.sums [i].displayValue, this.cMC, depth, xPos, this.categories [i].y, angle, valueStyleObj, false, 0, 0);
					//Apply filter
					this.styleM.applyFilters (valueObj.tf, this.objects.DATAVALUES);
					//Apply animation
					if (this.params.animation)
