...
*
* Column2DChart chart extends the SingleYAxis2DVerticalChart class to render a
* 2D Column Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.Column2DChart extends SingleYAxis2DVerticalChart 
{
	...
	*/
	function Column2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode : String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Single Series 2D Column Chart", Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "HGRID", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "DATAPLOT", "TOOLTIP", "VLINES");
	...
	*/
	private function returnDataAsObject (label : String, value : Number, color : String, alpha : String, ratio : String, toolText : String, link : String, showLabel : Number, showValue : Number, isDashed : Boolean) : Object 
	{
		//Create a container
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "SET")
					{
						...
						this.data [this.num] = this.returnDataAsObject (setName, setValue, setColor, setAlpha, setRatio, setToolText, setLink, setShowLabel, setShowValue, setDashed);
	...
	*/
	...
	{
		/**
		* For the 2D Column chart, we need to set defaults for the
		...
		*/
		...
		//Or, if we're plotting round edges, we necessarily apply shadow
	...
	*/
	private function calculatePoints ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			dataEndY = this.getAxisPosition (this.data [i].value, this.config.yMax, this.config.yMin, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
	...
	private function drawColumns ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.data [i].isDefined)
			{
				...
				if (this.params.useRoundEdges){
					var colIns:RoundColumn2D = new RoundColumn2D(colMC, this.data [i].w, this.data [i].h, this.roundEdgeRadius, this.data[i].color[0].toString(16), this.params.plotBorderThickness, this.params.plotBorderAlpha);
				}else{					
					var colIns:Column2D = new Column2D (colMC, this.data [i].w, this.data [i].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.data [i].color, this.data [i].alpha, this.data [i].ratio, this.params.plotFillAngle, false, this.data [i].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
				}
				//Draw the column
				colIns.draw ();
