 /**
* @class SplineArea2DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* SplineArea2DChart chart extends the SingleYAxis2DVerticalChart class to render a
* 2D Area Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.SplineArea2DChart extends SingleYAxis2DVerticalChart 
{
	...
	//object to store gradients calculated for each spline segments
	private var objGradientStore : Object;
	/**
	* Constructor function. We invoke the super class'
	* constructor and then set the objects for this chart.
	*/
	function SplineArea2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Single Series 2D Spline Area Chart", Logger.LEVEL.INFO);
		...
		this.data = new Array ();
		// create the container to store spline gradients for each spline segments
		this.objGradientStore = new Object ();
	...
	*/
	public function render () : Void 
	{
		...
		} else 
		{
			...
			//Draw line chart
			this.config.intervals.plot = setInterval (Delegate.create (this, drawSplineAreaChart) , this.timeElapsed);
			//Update timer
	...
	*/
	private function returnDataAsObject (label : String, value : Number, color : String, alpha : Number, toolText : String, link : String, showLabel : Number, showValue : Number, anchorSides : Number, anchorRadius : Number, anchorBorderColor : String, anchorBorderThickness : Number, anchorBgColor : String, anchorAlpha : Number, anchorBgAlpha : Number) : Object 
	{
		//Create a container
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "SET")
					{
						...
						var setColor : String = getFV (atts ["color"] , this.params.plotFillColor);
						...
						this.data [this.num] = this.returnDataAsObject (setName, setValue, setColor, setAlpha, setToolText, setLink, setShowLabel, setShowValue, setAnchorSides, setAnchorRadius, setAnchorBorderColor, setAnchorBorderThickness, setAnchorBgColor, setAnchorAlpha, setAnchorBgAlpha);
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		this.params.plotFillColor = formatColor (getFV (atts ["plotfillcolor"] , atts ["areabgcolor"] , this.defColors.get2DPlotFillColor (this.params.palette)));
		this.params.plotFillAlpha = getFN (atts ["plotfillalpha"] , atts ["areaalpha"] , 90);
		//Area Border Properties
		this.params.showPlotBorder = toBoolean (getFN (atts ["showplotborder"] , atts ["showareaborder"] , 1));
		this.params.plotBorderColor = formatColor (getFV (atts ["plotbordercolor"] , atts ["areabordercolor"] , this.defColors.get2DPlotBorderColor (this.params.palette)));
		this.params.plotBorderThickness = getFN (atts ["plotborderthickness"] , atts ["areaborderthickness"] , 2);
		...
		this.params.anchorBorderColor = formatColor (getFV (atts ["anchorbordercolor"] , this.params.plotFillColor));
	...
	*/
	private function calculatePoints ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			this.data [i].x = (this.num == 1) ? (this.elements.canvas.x + this.elements.canvas.w / 2) : (this.elements.canvas.x + this.params.canvasPadding + (interPointWidth * (i - 1)));
			//Set the y position
			this.data [i].y = this.getAxisPosition (this.data [i].value, this.config.yMax, this.config.yMin, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
			//Store max and min positions
			this.config.yMaxPos = (this.data [i].y > this.config.yMaxPos) ? this.data [i].y : this.config.yMaxPos;
			this.config.yMinPos = (this.data [i].y < this.config.yMinPos) ? this.data [i].y : this.config.yMinPos;
			//Store value textbox y position
			this.data [i].valTBY = this.data [i].y;
		}
		// call to set the gradient values required for drawing the spline curves if pixel position
		// difference along x-axis between two consecutive knots is greater than one (note that x is defined
		// irrespective of y is defined or not)
		if (this.data [2].x - this.data [1].x > 1)
		{
			this.setGradients ();
		}
	}
	...
	*/
	...
	/**
	* drawSplineAreaChart method draws the lines on the chart
	*/
	private function drawSplineAreaChart () : Void 
	{
		/**
		...
		*/
		...
		var matrix : Object = {
			matrixType : "box", w : (this.data [lastIndex].x - this.data [firstIndex].x) , h : (this.config.yMaxPos - this.config.yMinPos) , x : this.elements.canvas.x, y : this.elements.canvas.y, r : MathExt.toRadians (this.params.plotFillAngle)
		};
		...
		for (i = firstIndex; i < this.num; i ++)
		{
			...
			if (this.data [i].isDefined)
			{
				...
				{
					chartMC.lineStyle (this.params.plotBorderThickness, parseInt (this.params.plotBorderColor, 16) , this.params.plotBorderAlpha);
				} else 
				{
					chartMC.lineStyle ();
				}
				...
				{
					//Fill as gradient
					chartMC.beginGradientFill ("linear", [parseInt (this.data [i].color, 16) , parseInt (this.params.plotGradientColor, 16)] , [this.data [i].alpha, this.data [i].alpha] , [0, 255] , matrix);
				} else 
				{
					//Solid fill
					chartMC.beginFill (parseInt (this.data [i].color, 16) , this.data [i].alpha);
				}
				//Move to data position
				chartMC.moveTo (this.data [i].x, this.data [i].y);
				// if horizontally, pixel difference between related two points is greater than one
				if (this.data [nxt].x - this.data [i].x > 1)
				{
					//Draw spline curve to next point
					this.generateCurve (nxt, chartMC);
					// else, simply draw straight line between the points
					
				} else 
				{
					chartMC.lineTo (this.data [nxt].x, this.data [nxt].y);
				}
				//Now, we draw the line from data to bottom plane only if next data is undefined
				//or if it's last data
				if (this.params.showPlotBorder && ((nxt == lastIndex) || this.data [i + 2].isDefined == false))
				{
					chartMC.lineStyle (this.params.plotBorderThickness, parseInt (this.params.plotBorderColor, 16) , this.params.plotBorderAlpha);
				} else 
				{
				...
				chartMC.lineTo (this.data [nxt].x, this.config.basePlanePos);
				chartMC.lineStyle ();
				//Draw the line back to previous data x, base plane
				chartMC.lineTo (this.data [i].x, this.config.basePlanePos);
				//We draw the line back from current data to base plane
				//only if it's the first data or the previous data was null
				if (this.params.showPlotBorder && (i == firstIndex || this.data [i - 1].isDefined == false))
				{
					chartMC.lineStyle (this.params.plotBorderThickness, parseInt (this.params.plotBorderColor, 16) , this.params.plotBorderAlpha);
				} else 
				{
				...
				chartMC.lineTo (this.data [i].x, this.data [i].y);
				//End fill
				chartMC.endFill ();
				//Get maxY and minY
				maxY = (maxY == undefined || (this.data [i].y > maxY)) ? this.data [i].y : maxY;
				minY = (minY == undefined || (this.data [i].y < minY)) ? this.data [i].y : minY;
			}
		}
	...
	/**
	* setGradients is the method to calculate gradient
	* for curves between two successive knots.
	*/
	private function setGradients () : Void 
	{
		// reference of the object to store gradients in is assigned in a local variable
		var objGrad : Object = this.objGradientStore;
		// an array created to hold the pertinent data only thus required for gradients' calculations
		var arrKnot : Array = new Array ();
		// a counter variable initialised to zero ... will be updated and used w.r.t. pertinent data for set/get purposes
		var counter : Number = 0;
		// populating arrKnot with pertinent data by iterating and checking
		for (var u : Number = 1; u < this.data.length; ++ u)
		{
			// criteria of pertinence
			if ( ! isNaN (this.data [u].y))
			{
				// updating counter
				counter ++;
				// populated with an object for this data
				// index - the actual index position in original dataset
				// y - pixel y position of the point
				arrKnot [counter] = {
					index : Number (u) , y : this.data [u].y
				};
			}
		}
		// arrKnot is an array with starting index 1 and hence its length is
		// incremented by one over the actual length.
		// Below, each gradient is initialised to zero
		for (var i : Number = 1; i < arrKnot.length; ++ i)
		{
			// getting the original index for this data
			var t : Number = arrKnot [i]['index'];
			// initialised to zero
			objGrad ['D' + t] = 0;
		}
		var calcValue : Number;
		// The following iteration is a mathematical purpose - iterative method of approximation with faster convergence.
		// The more the number of iteration, the more precise the approximate solutions and vice-versa.
		for (var u : Number = 0; u < 10; ++ u)
		{
	...
	/**
	* generateCurve is the method to draw the curve between
	* two successive knots.
	*/
	private function generateCurve (index : Number, mcTarget : MovieClip) : Void 
	{
		// storing the reference of the repository of coordinates of the points
		var arrKnot : Array = this.data;
		var i : Number = index;
		var j : Number;
		// getting the index of last defined point
		for (var p = i - 1; p > 0; -- p)
		{
			if ( ! isNaN (arrKnot [p].y))
			{
				j = p;
				break;
			}
		...
		var slope_1 : Number = objGradientStore ['D' + (j)];
		var slope_2 : Number = objGradientStore ['D' + (i)];
		// calculating the four coefficients characterising the cubic polynomial
		var a1 : Number = arrKnot [j].y;
		var a2 : Number = slope_1;
		var a3 : Number = 3 * (arrKnot [i].y - arrKnot [j].y) - 2 * slope_1 - slope_2;
		var a4 : Number = 2 * (arrKnot [j].y - arrKnot [i].y) + slope_1 + slope_2;
		// pixel interval between two points horizontally
		var interval : Number = arrKnot [i].x - arrKnot [j].x;
		// reciprocal of the horizontal pixel interval
		var k : Number = 1 / interval;
		var t : Number, x : Number, y : Number;
		// The curve is drawn by joinig straight lines, with one pixel horizontal shifts.
		// Iterated to calculate coordinates and/for drawing the unit straight lines.
		for (var n : Number = 1; n <= interval; ++ n)
		{
			// t is a number such that 0<t<=1
			t = n * k;
			// incremented by one pixel each time
			x = arrKnot [j].x + n;
			// polynomial evaluation for ordinate w.r.t. the 4 coefficients
			y = a1 + a2 * t + a3 * t * t + a4 * t * t * t;
			// restricting the spline curve within the canvas
			if (y < this.elements.canvas.y)
			{
				// upper boundary restriction
				y = this.elements.canvas.y;
			} else if (y > this.elements.canvas.toY)
			{
				// lower boundary restriction
				y = this.elements.canvas.toY;
			}
			// unit line drawn
			mcTarget.lineTo (x, y);
		}
		// k, the reciprocal of the interval found above can be non-intezer, thus there may remain gap in curve at the end points.
		// To set this, we join the end of the curve with a straight line to the endpoint (a precautionary measure). It is to be noted that
		// each call of this method for draw of spline curve is preceded by moveTo(startpoint).
		mcTarget.lineTo (arrKnot [i].x, arrKnot [i].y);
	}
	/**
