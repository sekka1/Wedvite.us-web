 /**
* @class MSColumn3DLineDYChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* MSColumn3DLineDYChart chart extends the DualYAxis3DChart class to render a
* Mult-series 3D Column Line Dual Y Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.MSColumn3DLineDYChart extends DualYAxis3DChart 
{
	...
	*/
	function MSColumn3DLineDYChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Multi Series 3D Column Line Dual Y Chart", Logger.LEVEL.INFO);
	...
	*/
	private function calculatePoints ()
	{
		...
		var colWidth : Number = ((this.elements.canvas.w - this.params.canvasBgDepth - 2 * this.params.canvasPadding) * ((100 - this.params.plotSpacePercent) / 100)) / (this.num * this.numColDS);
		...
		if ( ! this.params.overlapColumns || this.numColDS * this.num == 1)
		{
		...
		//Get block width and store a copy of column Width as oColWidth
		//(as we'll later tweak oColWidth for overlap).
		var blockWidth : Number = colWidth * this.numColDS;
		var oColWidth : Number = colWidth;
		//Now, if we've to overlap columns (with number of columns in a dataset >1)
		//we need to re-calculate column width
		if (this.params.overlapColumns && this.numColDS > 1)
		{
			//Get the maximum possible x-space that the shadow of all columns in a block can take.
			var totalShadow : Number = (this.config.shifts.wShadow) * (this.numColDS);
			...
			colWidth = ((colWidth * this.numColDS) + (totalShadow * 0.5)) / (this.numColDS);
		...
		//Calculate space between two column blocks
		if (this.num > 2)
		{
			//If there are more than two blocks of columns on the chart, distribute them on the canvas equally .
			var blockSpace : Number = ((this.elements.canvasBg.w - 2 * this.params.canvasPadding) - (blockWidth * this.num)) / (this.num - 1);
		} else 
		{
			//If it's only 1 or 2 block of columns, we center align them instead of equal distribution.
			var blockSpace : Number = ((this.elements.canvasBg.w - 2 * this.params.canvasPadding) - (blockWidth * this.num)) / (this.num + 1);
		}
		//We finally have block width and space
		//Store it in config
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.num > 2)
			{
				this.categories [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (blockSpace * (i - 1)) + (blockWidth * (i - 0.5)) - this.config.shifts.xShift;
			}else
			{
				this.categories [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (blockSpace * i) + (blockWidth * (i - 0.5)) - this.config.shifts.xShift;
			}
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].parentYAxis == "P")
				{
					...
					if (this.num > 2)
					{
						this.dataset [j].data [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (blockSpace * (i - 1)) + (blockWidth * (i - 1)) + (oColWidth * (colCounter - 0.5)) - this.config.shifts.xShift;
					}else
					{
						this.dataset [j].data [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (blockSpace * i) + (blockWidth * (i - 1)) + (oColWidth * (colCounter - 0.5)) - this.config.shifts.xShift;
					}
					//Set the y position
					this.dataset [j].data [i].y = this.config.basePlanePos + this.config.shifts.yShift;
		...
		for (i = 1; i <= this.num; i ++)
		{
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.dataset [j].parentYAxis == "P" && this.dataset [j].data [i].isDefined)
				{
					...
					colIns = new Column3D (colMC, this.dataset [j].data [i].x, this.dataset [j].data [i].y, this.config.shifts.wFront, this.dataset [j].data [i].h, this.config.shifts.xShift, this.config.shifts.yShift, this.dataset [j].data [i].color, this.params.showPlotBorder, this.params.plotBorderColor, this.params.plotBorderAlpha);
					//Draw the column
					...
					if (this.params.animation)
					{
						...
						if (Math.abs (this.dataset [j].data [i].h) > (this.config.shifts.yShift * 1.5))
						{
							animY = 100;
						} 
						else
						...
						this.styleM.applyAnimation (colMC, this.objects.DATAPLOTCOLUMN, this.macro, this.dataset [j].data [i].x, 0, this.dataset [j].data [i].y, 0, 100, 100, animY, null);
					}
					//Apply filters
