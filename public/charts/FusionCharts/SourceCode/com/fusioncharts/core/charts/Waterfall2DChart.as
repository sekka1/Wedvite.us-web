/**
* @class Waterfall2DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* Waterfall2DChart chart extends the SingleYAxis2DVerticalChart class to render a
* 2D Waterfall Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.Waterfall2DChart extends SingleYAxis2DVerticalChart 
{
	...
	*/
	function Waterfall2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Single Series 2D Waterfall Chart", Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "HGRID", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "CONNECTORS", "DATAPLOT", "TOOLTIP", "VLINES");
		super.setChartObjects (_arrObjects);
		//Initialize the data structure (array) for single series chart
		this.data = new Array ();
		//Set default Value
		//Create the zeroth data element
		this.data[0] = returnDataAsObject ("", true, true, 0, 0);
		//Store the initial position
		this.data[0].h = 0;
		this.data[0].cumValue = 0;
	...
	*/
	public function render () : Void 
	{
		...
		} else 
		{
			...
			if (!(this.params.labelDisplay == "INSIDE"))
			{	//Draw labels
			...
			//Draw Connectors
			this.config.intervals.connector = setInterval (Delegate.create (this, drawConnector) , this.timeElapsed);
			if (this.params.labelDisplay == "INSIDE")
			{	//Draw labels
				this.config.intervals.labels = setInterval (Delegate.create (this, drawLabels) , this.timeElapsed);
			}
			...
			//Update timer
			this.timeElapsed += (this.params.animation) ? this.styleM.getMaxAnimationTime (this.objects.TRENDLINES, this.objects.TRENDVALUES, this.objects.VLINES, this.objects.DATAVALUES, this.objects.DATALABELS) : 0;
	...
	*/
	private function returnDataAsObject (label : String, dataSum:Boolean, dataCumulative:Boolean, index:Number, value : Number, dataCumValue:Number, color : String, alpha : String, ratio : String, toolText : String, link : String, showLabel : Number, showValue : Number, isDashed : Boolean) : Object 
	{
		//Create a container
		var dataObj : Object = new Object ();
		//Store the values
		dataObj.label = label;
		dataObj.isSum = dataSum;
		dataObj.cumulative = dataCumulative;
		dataObj.index = index;
		dataObj.value = value;
		dataObj.cumValue = dataCumValue;
		...
		dataObj.link = link;
		dataObj.showLabel = (showLabel == 1 && label != "") ? true : false;
	...
	private function parseXML () : Void 
	{
		...
		for (i = 0; i < arrDocElement.length; i ++)
		{
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				var setNode : XMLNode;
				//To store the sum for the Cumulative case
				var lastSum:Number = 0;
				var index:Number = 0;
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "SET")
					{
						...
						var setAsSum:Boolean = toBoolean (getFN (atts ["issum"] , 0));
						//Whether the sum will be displayed as the Cumulative Column or from the base plane
						var setCumulative:Boolean = false;
						if(setAsSum) {
							setCumulative = toBoolean (getFN (atts ["cumulative"] , 1));
							//Exclusive Case: When we have 2 consecutive sum with false cumulative attribute
							if(setCumulative == false && this.data[this.num-1].isSum == true) {
								//Set the current data as a cumulative data sum
								setCumulative = true;
							}
							//If the value is not given then we take as 0
							if(isNaN(setValue)) {
								setValue = 0;
							}
							if(setCumulative) {
								//reset the cumulative value
								setCumValue = this.data[this.num-1].cumValue;
								//Now, set the value.
								var setValue:Number = this.data[this.num-1].cumValue;
							} else {
								//Now, set the value.
								var setValue:Number = this.data[this.num-1].cumValue-lastSum;
								//reset the cumulative value
								setCumValue = this.data[this.num-1].cumValue;
							}
							lastSum = this.data[this.num-1].cumValue;
							index = this.num+1;
						} else {
							//Now, get value.
							...
							var setCumValue : Number = this.data[this.num-1].cumValue+setValue;
						}
						...
						//Based on set's value, set it's color, inheriting from positive and negatve color
						var setColor:String = getFV(atts ["color"], (setValue>=0)?(this.params.positiveColor):(this.params.negativeColor), this.defColors.getColor());
						...
						this.data [this.num] = this.returnDataAsObject (setName, setAsSum, setCumulative, index, setValue, setCumValue, setColor, setAlpha, setRatio, setToolText, setLink, setShowLabel, setShowValue, setDashed);
				...
				//Create a new object for the sum column if required.
				if(this.params.showSumAtEnd) {
					//Increase the number of nodes.
					this.num++;
					//Create a new object
					this.data[this.num] = this.returnDataAsObject(this.params.sumLabel, true, true, this.num-1, this.data[this.num-1].cumValue, this.data[this.num-1].cumValue, this.defColors.getColor() + ((this.params.plotGradientColor != "")?("," + this.params.plotGradientColor):""), this.params.plotFillAlpha, this.params.plotFillRatio, this.params.sumLabel, "", this.params.showLabels, this.params.showValues, this.params.plotBorderDashed);
					this.data[this.num-1].index = this.num-1;
				}
			}
		}
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		this.params.labelDisplay = (this.params.rotateLabels == "1") ? "INSIDE" : this.params.labelDisplay;
		...
		this.params.showConnectors = toBoolean (getFN (atts ["showconnectors"] , 1));
		this.params.connectorColor = getFV (atts ["connectorcolor"] , "000000");
		this.params.connectorAlpha = getFV (atts ["connectoralpha"] , 100);
		this.params.connectorThickness = getFV (atts ["connectorthickness"] , 1);
		//Connector is dashed?
		this.params.connectorDashed = toBoolean (getFN (atts ["connectordashed"] , 0));
		//Dash Properties
		this.params.connectorDashLen = getFN (atts ["connectordashlen"] , 2);
		this.params.connectorDashGap = getFN (atts ["connectordashgap"] , 2);
		//Parameter to check whether we need a sum column at the end
		this.params.showSumAtEnd = toBoolean (getFN (atts ["showsumatend"] , 1));
		this.params.sumLabel = getFV (atts ["sumlabel"] , "Total");
		//Positive and negative color for the columns
		this.params.positiveColor = atts["positivecolor"];
		this.params.negativeColor = atts["negativecolor"];
		// ------------------------- COSMETICS -----------------------------//		
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			} else 
			{
				...
				if (this.data [i].isDefined)
				{
					...
					maxValue = (this.data [i].cumValue > maxValue) ? this.data [i].cumValue : maxValue;
				}
			}
		}
		return maxValue;
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			} else 
			{
				...
				if (this.data [i].isDefined)
				{
					...
					minValue = (this.data [i].cumValue < minValue) ? this.data [i].cumValue : minValue;
				}
			}
		}
		return minValue;
	...
	*/
	private function setStyleDefaults () : Void 
	{
		...
		if (this.params.defaultAnimation)
		{
			...
			dataPlotAnim.param = "_y";
			dataPlotAnim.easing = "regular";
			dataPlotAnim.wait = 0;
			dataPlotAnim.start = "$canvasEndY";
			dataPlotAnim.duration = 1;
			//Over-ride
			this.styleM.overrideStyle (this.objects.DATAPLOT, dataPlotAnim, this.styleM.TYPE.ANIMATION, "_y");
			delete dataPlotAnim;
			//2. Y Scale for columns
			var dataPlotAnim = new StyleObject ();
			dataPlotAnim.name = "_SdDataPlotAnimYScale";
			...
			dataPlotAnim.wait = 0;
			dataPlotAnim.start = 0;
			dataPlotAnim.duration = 0.5;
			//Over-ride
			this.styleM.overrideStyle (this.objects.DATAPLOT, dataPlotAnim, this.styleM.TYPE.ANIMATION, "_alpha");
			delete dataPlotAnim;
			
			//Alpha for data Connector
			var dataConnAnim = new StyleObject ();
			dataConnAnim.name = "_SdDataConnectorAnimYScale";
			dataConnAnim.param = "_xscale";
			dataConnAnim.easing = "regular";
			dataConnAnim.wait = 0;
			dataConnAnim.start = 0;
			dataConnAnim.duration = 1;
			//Over-ride
			this.styleM.overrideStyle (this.objects.CONNECTORS, dataConnAnim, this.styleM.TYPE.ANIMATION, "_xscale");
			delete dataConnAnim;
		}
		//-----------------------------------------------------------------//
	...
	*/
	private function calculatePoints ()
	{
		...
		//2. Inside - no staggering - no wrapping
		//3. Wrapped - no staggering - no rotation
		//4. Staggered - no wrapping - no rotation
		//5. Rotated - no staggering - no wrapping
		...
		{
			//Case 5: If the labels are rotated, we iterate through all the string labels
		...
		} else if (this.params.labelDisplay == "INSIDE")
		{
			//Set the max label height as 0.
			this.config.labelAreaHeight = 0;
		} else if (this.params.labelDisplay == "WRAP")
		{
			//Case 3 (WRAP): Create all the labels on the chart. Set width as
		...
		{
			//Case 1,4: Normal or Staggered Label
		...
		var dataEndY : Number;
		//Set the initial position as the base plane position
		this.data[0].y = this.config.basePlanePos;
		//Now, store the positions of the columns
		for (i = 1; i <= this.num; i ++)
		{
			//X-Position
			this.data [i].x = this.elements.canvas.x + (interColSpace * i) + columnWidth * (i - 0.5);
			//Check if we need the column from the base plane position
			if(this.data[i].isSum && this.data[i].cumulative) {
				//If Cumulative then we start from base plane position
				this.data[i].y = this.config.basePlanePos;
			}else if(this.data[i].isSum && !this.data[i].cumulative) {
				//If Cumulative then we start from base plane position
				this.data[i].y = this.data [this.data[i-1].index].y;
			}
			else {
				//else Set the y position as the height of the last column
				this.data [i].y = this.data [i-1].y-this.data [i-1].h;
			}
			if(!this.params.setAdaptiveYMin) {
				//Height for each column
				dataEndY = this.getAxisPosition (Number(this.data [i].value), this.config.yMax, this.config.yMin, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
				//Negate to cancel Flash's reverse Y Co-ordinate system
				this.data [i].h = -(dataEndY-this.config.basePlanePos);
			} else {
				//Height for each column
				dataEndY = this.getAxisPosition (Number(this.data [i].cumValue), this.config.yMax, this.config.yMin, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
				//Negate to cancel Flash's reverse Y Co-ordinate system
				this.data [i].h = -(dataEndY-this.data [i].y);
			}
			//Width
			this.data [i].w = columnWidth;
			//Store value textbox y position
			this.data [i].valTBY = this.data [i].y-this.data [i].h;
		}
	}
	/**
	...
	*/
	private function allotDepths () : Void
	{
		...
		//Data Connector
		this.dm.reserveDepths ("CONNECTORS", this.num);
		//Data Columns
	...
	*/
	private function drawColumns ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.data [i].isDefined)
			{
				...
				colIns = new Column2D (colMC, this.data [i].w, this.data [i].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.data [i].color, this.data [i].alpha, this.data [i].ratio, this.params.plotFillAngle, true, this.data [i].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
				//Draw the column
				colIns.draw ();
				//Set x and y position
				colMC._x = this.data [i].x;
				colMC._y = this.data [i].y;
				//Set the alpha of entire column as the first alpha in list of alpha
				colMC._alpha = this.data [i].alpha [0];
				//Apply animation
				if (this.params.animation)
				{
					this.styleM.applyAnimation (colMC, this.objects.DATAPLOT, this.macro, this.data [i].x, 0, this.data [i].y, 0, 100, 100, 100, null);
				}
				//Apply filters
	...
	/**
	* drawConnector method draws the connector between the columns
	*/
	private function drawConnector()
	{
		//Check whether we need to draw the connectors
		if(this.params.showConnectors) { 
			//Initialize the variables
			var i:Number, nxt:Number = -1, j:Number;
			//Get the depth
			...
			{
				//Create an empty movie clip for this column
				var connectorMC:MovieClip = this.cMC.createEmptyMovieClip ("Connector_"+i, depth++);
				//Set the linestyle
				connectorMC.lineStyle(this.params.connectorThickness, parseInt(this.params.connectorColor, 16), this.params.connectorAlpha); 
				//If defined
				if (this.data [i].isDefined)
				{
					//Get next Index
					...
					if (this.data [nxt].isDefined == false)
					{
					...
					var w:Number = Math.abs(this.data [i].x-this.data [nxt].x)+this.config.columnWidth;
					//If the next column is not the sum then we draw the connector
					if(!this.data [nxt].isSum) {
						if (this.params.connectorDashed){
							//Draw a dashed line
							DrawingExt.dashTo (connectorMC, -w/2, 0, w/2, 0, this.params.connectorDashLen, this.params.connectorDashGap);
						} else 
						{
							//Draw line from top of one column to bottom of next column
							connectorMC.moveTo(-w/2, 0);
							connectorMC.lineTo(w/2, 0);
						}
						//Set the position
						connectorMC._x = (this.data [i].x+this.data [nxt].x)/2;
						connectorMC._y = this.data [i].y - this.data [i].h;
						//Apply animation
						if (this.params.animation)
						{
							this.styleM.applyAnimation (connectorMC, this.objects.CONNECTORS, this.macro, connectorMC._x, 0, connectorMC._y, 0, 100, 100, 100, null);
						}
						//Apply filters
						this.styleM.applyFilters (connectorMC, this.objects.CONNECTORS);
					}
				}
			}
		}
		//Clear interval
		clearInterval (this.config.intervals.connector);
	}
	/**
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.data [i].isDefined && this.data [i].showValue)
			{
				...
				if (this.params.placeValuesInside)
				{
					...
					vAlign = (this.data [i].h >= 0) ?"bottom" : "top";
					yPos = (this.data [i].h >= 0) ? (this.data [i].valTBY + this.params.valuePadding) : (this.data [i].valTBY - this.params.valuePadding);
				}else
				{
					vAlign = (this.data [i].h >= 0) ?"top" : "bottom";
					yPos = (this.data [i].h >= 0) ? (this.data [i].valTBY - this.params.valuePadding) : (this.data [i].valTBY + this.params.valuePadding);
				}
				//Align position
				align = "center";
				...
				valueObj = createText (false, this.data [i].displayValue, this.cMC, depth, this.data [i].x, yPos, angle, valueStyleObj, false, 0, 0);
				//Now, we need to adjust those labels which are falling out of height.
				if (this.params.placeValuesInside && ((this.params.valuePadding + valueObj.height) >= Math.abs (this.data [i].h)))
				{
					...
					if ( ! this.params.rotateValues)
					{
						...
						{
							//Show above
							valueObj.tf._y = this.data [i].y - this.data [i].h - valueObj.height - this.params.valuePadding;
						}else
						{
							//Negative Column - show below
							valueObj.tf._y = this.data [i].y + Math.abs (this.data [i].h) + this.params.valuePadding;
						}
					}
					else
					{
						...
						{
							//Show above
							valueObj.tf._y = this.data [i].y - this.data [i].h - this.params.valuePadding;
						}else
						{
							//Negative Column - show below
							valueObj.tf._y = this.data [i].y + Math.abs (this.data [i].h) + valueObj.height + this.params.valuePadding;
						}
					}
				}
				//Next, we adjust those labels are falling out of top canvas area
				//For Positive columns only
				if (( ! this.params.placeValuesInside) && (this.data [i].h >= 0) && ((yPos - valueObj.height) <= this.elements.canvas.y))
				{
				...
				if (( ! this.params.placeValuesInside) && (this.data [i].h < 0) && ((yPos + valueObj.height) >= this.elements.canvas.toY))
				{
