/**
* @class ScrollColumn2DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0.3
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* ScrollColumn2DChart chart extends the SingleYAxis2DVerticalChart class to render a
* Multi-series 2D Column Chart with scrolling feature.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.ScrollColumn2DChart extends SingleYAxis2DVerticalChart 
{
	...
	*/
	function ScrollColumn2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Scroll Column 2D Chart", Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "HGRID", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "DATAPLOT", "TOOLTIP", "VLINES", "LEGEND", "SCROLLPANE");
	...
	*/
	private function calculatePoints ()
	{
		...
		if(this.params.numVisiblePlot < 2 ||  this.params.numVisiblePlot > (this.num*this.numDS))  {
			this.params.numVisiblePlot = (this.num*this.numDS);
		}
		//Condition to check whether scroll is not required.
		if(this.params.numVisiblePlot < (this.num*this.numDS))  {
		...
		} else if (this.params.labelDisplay == "WRAP")
		{
			...
			//(canvasWidth*((this.num*this.numDS)/this.params.numVisiblePlot)/ finalVisibleCount)
			//(basically calculating the ratio of visible to total area)
			//Set max height as 50% of available canvas height at this point of time. Find all
			//and select the max one.
			var maxLabelWidth : Number = (canvasWidth*((this.num*this.numDS)/this.params.numVisiblePlot)/ finalVisibleCount);
		...
		//Block Width
		var totColWidth : Number =  columnWidth*this.num*this.numDS;
		if(this.scrollRequired) { 
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			this.categories [i].x = (interBlockSpace * i) + (columnWidth * this.numDS * (i - 0.5));
			for (j = 1; j <= this.numDS; j ++)
			{
				//X-Position
				this.dataset [j].data [i].x = (interBlockSpace * i) + columnWidth * (j - 0.5) + (columnWidth * this.numDS * (i - 1));
				//Set the y position
				this.dataset [j].data [i].y = this.config.basePlanePos - this.elements.canvas.y;
				...
				this.dataset [j].data [i].valTBY = dataEndY -  this.elements.canvas.y;
			}
		}
	}
	/**
	...
	*/
	private function createContainerMC():Void{
		...
		this.scrollRefMC.lineTo((this.categories[this.num].x+this.config.interBlockSpace+(this.config.columnWidth * this.numDS * 0.5)), 0);		
	}
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined)
				{
					...
					if (this.params.animation)
					{
						this.styleM.applyAnimation (colMC, this.objects.DATAPLOT, this.macro, null, 0, null, 0, 100, 100, 100, null);
					}
					//Apply filters
