/**
* @class ErrorBar2DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* ErrorBar2DChart chart extends the SingleYAxis2DVerticalChart class to render a
* Multi-series 2D Error Bar Chart.
*/
//Import Chart class
...
//errorBars
import com.fusioncharts.core.chartobjects.ErrorBar
//Legend Class
...
class com.fusioncharts.core.charts.ErrorBar2DChart extends SingleYAxis2DVerticalChart 
{
	...
	*/
	function ErrorBar2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Error Bar Chart", Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "HGRID", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "DATAPLOT", "ERRORBARS", "TOOLTIP", "VLINES", "LEGEND");
	...
	*/
	public function render () : Void 
	{
		...
		} else 
		{
			...
			//Draw errorBars
			this.config.intervals.error = setInterval (Delegate.create (this, drawErrorBars) , this.timeElapsed);
			//Data Values
	...
	*	@param	errorValue	Error value for data column.
	...
	*/
	private function returnDataAsObject (value : Number, errorValue : Number, color : String, alpha : String, ratio : String, toolText : String, errorToolText : String, link : String, showValue : Number, isDashed : Boolean) : Object 
	{
		//Create a container
		var dataObj : Object = new Object ();
		//Store the values
		dataObj.value = value;
		dataObj.errorValue = errorValue;
		...
		dataObj.errorToolText = errorToolText
		dataObj.link = link;
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						//---------- Errorbars--------------//
						this.dataset [this.numDS].errorBarColor = getFV (dsAtts ["errorbarcolor"] , this.params.errorBarColor);
						this.dataset [this.numDS].errorBarAlpha = getFV (dsAtts ["errorbaralpha"] , this.params.errorBarAlpha);
						this.dataset [this.numDS].errorBarThickness = getFV (dsAtts ["errorbarthickness"] , this.params.errorBarThickness);
						//----------------------------------//
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								var setErrorValue : Number = this.getSetValue (atts ["errorvalue"]);
								...
								var setErrorToolText : String = String (setErrorValue)
								...
								this.dataset [this.numDS].data [setCount] = this.returnDataAsObject (setValue, setErrorValue, setColor, setAlpha, setRatio, setToolText, setErrorToolText, setLink, setShowValue, setDashed);
							}
						}
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		// --------------------- ERRORBARS ------------------------- //
		this.params.halfErrorBar = toBoolean (getFN (atts ["halferrorbar"] , 1));
		this.params.errorBarColor = getFV (atts ["errorbarcolor"] , 'AAAAAA');
		this.params.errorBarAlpha = getFN (atts ["errorbaralpha"] , 100);
		this.params.errorBarThickness = getFN (atts ["errorbarthickness"] , 1);
		this.params.errorBarWidthPercent = getFN (atts ["errorbarwidthpercent"] , 70)
		// ------------------------- COSMETICS -----------------------------//
	...
	* in the data w.r.t the error values.
	* @return	The maximum value present in the data provided.
	*/
	private function getMaxDataValue():Number {
		//initialised by minimum possible value
		var maxValue:Number = Number.MIN_VALUE;
		var checkValue:Number, dataValue:Number, errorValue:Number;
		var i:Number, j:Number;
		for (i=1; i<=this.numDS; i++) {
			for (j=1; j<=this.num; j++) {
				if(this.dataset[i].data[j].isDefined){
					dataValue = this.dataset[i].data[j].value;
					errorValue = this.dataset[i].data[j].errorValue;
					errorValue = (isNaN(errorValue))?0:errorValue
					//if error bar is half
					if (this.params.halfErrorBar) {
						checkValue = Math.max(dataValue, dataValue+errorValue);
						//else error bar is full
					} else {
						checkValue = Math.max(dataValue, dataValue+Math.abs(errorValue));
					}
					//checking for the maximum value
					maxValue = (checkValue>maxValue) ? checkValue : maxValue;
				}
			}
		}
	...
	*/
	private function getMinDataValue():Number {
		//initialised by maximum possible value
		...
		for (i=1; i<=this.numDS; i++) {
			for (j=1; j<=this.num; j++) {
				if(this.dataset[i].data[j].isDefined){
					...
					//if error bar is half
					if (this.params.halfErrorBar) {
						checkValue = Math.min(dataValue, dataValue+errorValue);
						//else error bar is full
					} else {
						checkValue = Math.min(dataValue, dataValue-Math.abs(errorValue));
					}
					//checking for the minimum value
					minValue = (checkValue<minValue) ? checkValue : minValue;
				}
			}
		}
	...
	*/
	private function setStyleDefaults () : Void 
	{
		...
		if (this.params.defaultAnimation)
		{
			...
			delete dataPlotAnim;
			var errorBarsAnim = new StyleObject ();
			errorBarsAnim.name = "_SdErrorBarAnim";
			errorBarsAnim.param = "_alpha";
			errorBarsAnim.easing = "regular";
			errorBarsAnim.wait = 0;
			errorBarsAnim.start = 0;
			errorBarsAnim.duration = 1;
			//Over-ride
			this.styleM.overrideStyle (this.objects.ERRORBARS, errorBarsAnim, this.styleM.TYPE.ANIMATION, "_alpha");
			delete errorBarsAnim;
		}
		//-----------------------------------------------------------------//
	...
	private function calculatePoints ()
	{
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				this.dataset [i].data [j].errorToolText = formatNumber (this.dataset [i].data [j].errorValue, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix);
				//Tool tip text.
		...
		var dataEndY : Number, errorBarH : Number;
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				//------------- ERRORBAR ---------------------//
				//Height of half-errorBar for this data
				errorBarH = this.getAxisPosition (this.dataset [j].data [i].errorValue, this.config.yMax, this.config.yMin, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
				//height of half-errorBar
				this.dataset [j].data [i].errorH = - (errorBarH - this.config.basePlanePos);
				//width of errorBar
				this.dataset [j].data [i].errorW = this.dataset [j].data [i].w * this.params.errorBarWidthPercent / 100
				//x of half-errorBar
				this.dataset [j].data [i].errorX = this.dataset [j].data [i].x
				//y of half-errorBar
				this.dataset [j].data [i].errorY = dataEndY
				//-----------------------------------//
	...
	*/
	private function allotDepths () : Void
	{
		...
		//Error bars
		this.dm.reserveDepths ("ERRORBARS", this.num * this.numDS);
		//Zero Plane
	...
	*/
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined)
				{
					...
					colIns = new Column2D (colMC, this.dataset [i].data [j].w, this.dataset [i].data [j].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.dataset [i].data [j].color, this.dataset [i].data [j].alpha, this.dataset [i].data [j].ratio, this.params.plotFillAngle, false, this.dataset [i].data [j].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
					//Draw the column
					colIns.draw ();
	...
	/**
	* drawErrorBars method draws the errorBars on the chart
	*/
	private function drawErrorBars ()
	{
		//Variables
		var mcErrorBar : MovieClip;
		var errIns : ErrorBar;
		var depth : Number = this.dm.getDepth ("ERRORBARS");
		var i : Number, j : Number;
		//Create function storage containers for Delegate functions
		var fnRollOver : Function, fnClick : Function;
		//Iterate through all columns
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				//If data defined and error specified
				if (this.dataset [i].data [j].isDefined && this.dataset [i].data [j].errorH)
				{
					//Create an empty movie clip for this column
					mcErrorBar = this.cMC.createEmptyMovieClip ("ErrorBar_" + i + "_" + j, depth);
					//Create column instance
					errIns = new ErrorBar (mcErrorBar, this.dataset [i].data [j].errorW, this.dataset [i].data [j].errorH, this.params.halfErrorBar, this.dataset [i].errorBarColor, this.dataset [i].errorBarAlpha, this.dataset [i].errorBarThickness);
					//Draw the column
					errIns.draw ();
					//Set x and y position
					mcErrorBar._x = this.dataset [i].data [j].errorX;
					mcErrorBar._y = this.dataset [i].data [j].errorY;
					//Don't use hand cursor
					mcErrorBar.useHandCursor = false;
					//Apply animation
					if (this.params.animation)
					{
						this.styleM.applyAnimation (mcErrorBar, this.objects.ERRORBARS, this.macro, this.dataset [i].data [j].x, 0, (this.dataset [i].data [j].value >= 0) ? (this.dataset [i].data [j].y - this.dataset [i].data [j].h) : (this.dataset [i].data [j].y + this.dataset [i].data [j].height) , 0, 100, 100, 100, null);
					}
					//Apply filters
					this.styleM.applyFilters (mcErrorBar, this.objects.ERRORBARS);
					//Event handlers for tool tip
					if (this.params.showToolTip)
					{
						//Create Delegate for roll over function columnOnRollOver
						...
						fnRollOver.dsindex = i;
						fnRollOver.index = j;
						fnRollOver.isColumn = false;
						//Assing the delegates to movie clip handler
						mcErrorBar.onRollOver = fnRollOver;
						//Set roll out and mouse move too.
						mcErrorBar.onRollOut = mcErrorBar.onReleaseOutside = Delegate.create (this, columnOnRollOut);
						mcErrorBar.onMouseMove = Delegate.create (this, columnOnMouseMove);
					}
					//Increase depth
					depth ++;
				}
		...
		//Clear interval
		clearInterval (this.config.intervals.error);
	}
	/**
	...
	*/
	private function columnOnRollOver () : Void
	{
		...
		var isColumn : Boolean = arguments.caller.isColumn;
		if (isColumn)
		{
			//Set tool tip text for column
			this.tTip.setText (this.dataset [dsindex].data [index].toolText);
		} else 
		{
			this.tTip.setText (this.dataset [dsindex].data [index].errorToolText);
		}
		//Show the tool tip
		this.tTip.show ();
	}
