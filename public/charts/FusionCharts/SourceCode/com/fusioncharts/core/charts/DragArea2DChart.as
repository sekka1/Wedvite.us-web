/**
* @class DragArea2DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* DragArea2DChart chart extends the Drag2DVerticalChart class to render a
...
class com.fusioncharts.core.charts.DragArea2DChart extends com.fusioncharts.core.Drag2DVerticalChart {
	...
	*/
	function DragArea2DChart(targetMC:MovieClip, depth:Number, width:Number, height:Number, x:Number, y:Number, debugMode:Boolean, lang:String, scaleMode:String, registerWithJS:Boolean, DOMId:String) {
	...
	*/
	...
	/**
	 * drawAreaSlice method draw a single line on the chart
	*/
	private function drawAreaSlice(chartMC:MovieClip, m:Number, i:Number, nxt:Number):Void {
		//Create matrix object for gradient fill
		var matrix : Object = {
			matrixType : "box", w : (this.dataset [m].data [this.dataset[m].lastIndex].x - this.dataset [m].data [this.dataset[m].firstIndex].x) , h : (this.config.yMaxPos - this.config.yMinPos) , x : this.elements.canvas.x, y : this.elements.canvas.y, r : MathExt.toRadians (this.params.plotFillAngle)
		};
		...
		if (this.dataset [m].showPlotBorder)
		{
			...
			if (this.dataset [m].data [i].dashed)
			{
				...
				if (this.dataset [m].showPlotBorder && ((nxt == this.dataset[m].lastIndex) || (this.params.connectNullData == false && this.dataset [m].data [i + 2].isDefined == false)))
				{
				...
				//We draw the line back from current data to base plan
				//only if it's the first data or the previous data was null (& connectNull=false)
				if (this.dataset [m].showPlotBorder && (i == this.dataset[m].firstIndex || (this.params.connectNullData == false && this.dataset [m].data [i - 1].isDefined == false)))
				{
	...
	private function drawAreaChart () : Void 
	{
		...
		*/
		...
		for (m = 1; m <= this.numDS; m ++)
		{
			...
			this.dataset[m].firstIndex = this.num + 1;
			this.dataset[m].lastIndex = - 1;
			...
			for (i = this.dataset[m].firstIndex; i < this.num; i ++)
			{
			...
			subHolderMC._y = -(maxY) + (maxY - this.config.basePlanePos);
	...
	*/
	private function setContextMenu():Void {
		...
		var snapCMI:ContextMenuItem = new ContextMenuItem("Snap To Div",  fnOnSnap, true, !this.params.snapToDiv);
		...
		if(this.config.enableDragging && this.dataset[dsindex].data[index].isDraggable) {
			...
			anchorMC.onEnterFrame = function() {
				...
				if (classRef.dataset[dsindex].data[prv].isDefined && index != 1 && prv != -1) {
					...
					//If its the first set of the dragged dataset/or the prev Dataset is NULL
					dragBorderMC.moveTo(this._x, classRef.config.basePlanePos);
					dragBorderMC.lineTo(this._x, this._y);
				}
	...
	private function dataOnRelease():Void {
		...
		if(this.config.enableDragging && this.dataset[dsindex].data[index].isDraggable) {
			...
			if(prv != -1) {
				...
				this.drawAreaSlice(chartMC, dsindex, prv, index);
			}
			...
			if(nxt != -1) {
				//Now redraw the next line
				this.drawAreaSlice(chartMC, dsindex, index, nxt);
			}
		}
	}
	/**
