...
class com.fusioncharts.core.charts.MSStackedColumn2DLineDYChart extends DualYAxis2DChart 
{
	...
	*/
	function MSStackedColumn2DLineDYChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "HGRID", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "DATAPLOTCOLUMN", "DATAPLOTLINE", "ANCHORS", "TOOLTIP", "VLINES", "LEGEND");
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
							...
							if (subDataSetNodes [k].nodeName.toUpperCase () == "DATASET")
							{
								...
								for (l = 0; l < arrLevel2Nodes.length; l ++)
								{
									if (arrLevel2Nodes [l].nodeName.toUpperCase () == "SET")
									{
										...
										this.dataset [this.numDS].dataset [sDSCount].data [setCount] = this.returnDataAsObject (setValue, setColor, setAlpha, setRatio, setToolText, setLink, setShowValue, setDashed);
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "LINESET")
					{
						...
						this.ldataset [this.numLDS].color = formatColor (getFV (dsAtts ["color"] , this.defColors.getColor ()));
						...
						//Dash Properties
						this.ldataset [this.numLDS].dashed = toBoolean (getFN (dsAtts ["dashed"] , this.params.plotBorderDashed));
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								//Store all these attributes as object.
								this.ldataset [this.numLDS].data [setCount] = this.returnDataAsLineObject (setValue, setColor, Number (setAlpha) , setToolText, setLink, setShowValue, setDashed, setAnchorSides, setAnchorRadius, setAnchorBorderColor, setAnchorBorderThickness, setAnchorBgColor, setAnchorAlpha, setAnchorBgAlpha);
							}
						}
	...
	*/
	private function calculateAxisLimits () : Void 
	{
		...
		this.getSAxisLimits (this.getMaxLineDataValue () , this.getMinLineDataValue () , ! this.params.setAdaptiveSYMin);
	}
	/**
	...
	*/
	private function calculatePoints ()
	{
		...
		for (i = 1; i <= this.numLDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				this.ldataset [i].data [j].displayValue = formatNumber (this.ldataset [i].data [j].value, this.params.sFormatNumber, this.params.sDecimals, this.params.sForceDecimals, this.params.sFormatNumberScale, this.params.sDefaultNumberScale, this.config.snsv, this.config.snsu, this.params.sNumberPrefix, this.params.sNumberSuffix);
				//Tool tip text.
				//Preferential Order - Set Tool Text (No concatenation) > SeriesName + Cat Name + Value
		...
		if (this.params.showLegend)
		{
			...
			for (i = 1; i <= this.numLDS; i ++)
			{
				...
				{
					lgnd.addItem (this.ldataset [i].seriesName, this.ldataset [i].color);
				}
			}
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				for (k = 1; k <= this.dataset [j].numSDS; k ++)
				{
					...
					if (this.dataset [j].dataset [k].data [i].isDefined)
					{
						...
						dataEndY = this.getAxisPosition (this.dataset [j].dataset [k].data [i].value, this.config.PYMax, this.config.PYMin, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
