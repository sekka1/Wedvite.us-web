 /**
* @class MSSplineArea2DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* MSSplineArea2DChart chart extends the SingleYAxis2DVerticalChart class to render a
...
class com.fusioncharts.core.charts.MSSplineArea2DChart extends SingleYAxis2DVerticalChart 
{
	...
	private var arrGradientStore : Array;
	/**
	* Constructor function. We invoke the super class'
	* constructor and then set the objects for this chart.
	*/
	function MSSplineArea2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Multi-Series 2D Spline Area Chart", Logger.LEVEL.INFO);
		...
		this.arrGradientStore = new Array ()
	...
	*/
	private function calculatePoints ()
	{
		...
		// difference along x-axis between two consecutive knots is greater than one (note that x is
		// defined irrespective of y is defined or not)
		var delX : Number = this.dataset [1].data [2].x - this.dataset [1].data [1].x
		if (delX > 1)
		{
			this.setGradients ();
		}
	...
	*/
	...
	/**
	* drawSplineAreaChart method draws the area on the chart
	*/
		...
		for (m = 1; m <= this.numDS; m ++)
		{
			...
			for (i = firstIndex; i < this.num; i ++)
			{
				...
				if (this.dataset [m].data [i].isDefined)
				{
					...
					if (this.dataset [m].data [nxt].x - this.dataset [m].data [i].x > 1)
					{
						//Draw spline curve to next point
						this.generateCurve (nxt, m, chartMC)
					...
					if (this.dataset [m].showPlotBorder && ((nxt == lastIndex) || this.dataset [m].data [i + 2].isDefined == false))
					{
						chartMC.lineStyle (this.dataset [m].plotBorderThickness, parseInt (this.dataset [m].plotBorderColor, 16) , this.dataset [m].plotBorderAlpha);
					} else 
					{
					...
					if (this.dataset [m].showPlotBorder && (i == firstIndex || this.dataset [m].data [i - 1].isDefined == false))
					{
						chartMC.lineStyle (this.dataset [m].plotBorderThickness, parseInt (this.dataset [m].plotBorderColor, 16) , this.dataset [m].plotBorderAlpha);
					} else 
					{
	...
	*/
	...
	{
		// Iterating once for each data series
		for (var q = 1; q < this.dataset.length; ++ q)
		{
			// an object created to bundle all gradients for a data series
			this.arrGradientStore [q] = new Object ();
			// reference of the object to store gradients in is assigned in a local variable
			var objGrad : Object = this.arrGradientStore [q];
			...
			for (var u = 1; u < this.dataset [q].data.length; ++ u)
			{
				// criteria of pertinence
				if ( ! isNaN (this.dataset [q].data [u].y))
				{
					// updating counter
					counter ++;
					...
					arrKnot [counter] = {
						index : Number (u) , y : this.dataset [q].data [u].y
					};
				}
			}
	...
	*/
	private function generateCurve (index : Number, setIndex : Number, mcTarget : MovieClip) : Void 
	{
		// storing the reference of the repository of coordinates of the points
		var arrKnot : Array = this.dataset [setIndex].data;
		var i : Number = index;
		var j : Number;
		...
		var slope_1 : Number = arrGradientStore [setIndex]['D' + j];
		var slope_2 : Number = arrGradientStore [setIndex]['D' + i];
