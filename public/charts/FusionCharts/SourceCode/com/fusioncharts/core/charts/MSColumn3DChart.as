 /**
* @class MSColumn3DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
*
* MSColumn3DChart chart extends the SingleYAxis3DChart class to render a
* Mult-series 3D Column Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.MSColumn3DChart extends SingleYAxis3DChart 
{
	...
	*/
	function MSColumn3DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "DATAPLOT", "TOOLTIP", "VLINES", "LEGEND");
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		//Show column shadows
		this.params.showColumnShadow = toBoolean (getFN (atts ["showshadow"] , atts ["showcolumnshadow"] , this.params.use3DLighting));
	...
	*/
	private function calculatePoints ()
	{
		...
		var colWidth : Number = ((this.elements.canvas.w - this.params.canvasBgDepth - 2 * this.params.canvasPadding) * ((100 - this.params.plotSpacePercent) / 100)) / (this.num * this.numDS);
		...
		if ( ! this.params.overlapColumns || this.numDS * this.num == 1)
		{
		...
		var blockWidth : Number = colWidth * this.numDS;
		...
		if (this.params.overlapColumns && this.numDS > 1)
		{
			//Get the maximum possible x-space that the shadow of all columns in a block can take.
			var totalShadow : Number = (this.config.shifts.wShadow) * (this.numDS);
			...
			colWidth = ((colWidth * this.numDS) + (totalShadow * 0.5)) / (this.numDS);
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.num > 2)
				{
					this.dataset [j].data [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (blockSpace * (i - 1)) + (blockWidth * (i - 1)) + (oColWidth * (j - 0.5)) - this.config.shifts.xShift;
				}else
				{
					this.dataset [j].data [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (blockSpace * i) + (blockWidth * (i - 1)) + (oColWidth * (j - 0.5)) - this.config.shifts.xShift;
				}
				//Set the y position
