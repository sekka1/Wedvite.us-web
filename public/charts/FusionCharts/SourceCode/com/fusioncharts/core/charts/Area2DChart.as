...
*
* Area2DChart chart extends the SingleYAxis2DVerticalChart class to render a
* 2D Area Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.Area2DChart extends SingleYAxis2DVerticalChart 
{
	...
	*/
	function Area2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Single Series 2D Area Chart", Logger.LEVEL.INFO);
	...
	*/
	private function calculatePoints ()
	{
		...
		{
			//All negative numbers - Set max value as base plane position
		...
		*/
		...
		for (i = firstIndex; i < this.num; i ++)
		{
			...
			if (this.data [i].isDefined)
			{
				...
				if (this.params.connectNullData == false && this.data [nxt].isDefined == false)
				{
				...
				if (this.params.showPlotBorder)
				{
					...
					{
						//Draw a dashed line
						DrawingExt.dashTo (chartMC, this.data [i].x, this.data [i].y, this.data [nxt].x, this.data [nxt].y, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
						...
						if (this.params.showPlotBorder && ((nxt == lastIndex) || (this.params.connectNullData == false && this.data [i + 2].isDefined == false)))
						{
							DrawingExt.dashTo (chartMC, this.data [nxt].x, this.data [nxt].y, this.data [nxt].x, this.config.basePlanePos, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
						}
						//Draw the line back to previous data x, base plane
						DrawingExt.dashTo (chartMC, this.data [nxt].x, this.config.basePlanePos, this.data [i].x, this.config.basePlanePos, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
						//We draw the line back from current data to base plan
						//only if it's the first data or the previous data was null (& connectNull=false)
						if (this.params.showPlotBorder && (i == firstIndex || (this.params.connectNullData == false && this.data [i - 1].isDefined == false)))
						{
							DrawingExt.dashTo (chartMC, this.data [i].x, this.config.basePlanePos, this.data [i].x, this.data [i].y, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
						}
					} else 
					{
						//Draw point to next line
						chartMC.lineTo (this.data [nxt].x, this.data [nxt].y);
						chartMC.moveTo (this.data [nxt].x, this.data [nxt].y);
						//Now, we draw the line from data to bottom plane only if next data is undefined
						//(and connectNullData == false) or if it's last data
						if (this.params.showPlotBorder && ((nxt == lastIndex) || (this.params.connectNullData == false && this.data [i + 2].isDefined == false)))
						{
						...
						//Draw the line back to previous data x, base plane
						chartMC.moveTo (this.data [nxt].x, this.config.basePlanePos);
						chartMC.lineTo (this.data [i].x, this.config.basePlanePos);
						//We draw the line back from current data to base plan
						//only if it's the first data or the previous data was null (& connectNull=false)
						if (this.params.showPlotBorder && (i == firstIndex || (this.params.connectNullData == false && this.data [i - 1].isDefined == false)))
						{
							chartMC.moveTo (this.data [i].x, this.config.basePlanePos);
							chartMC.lineTo (this.data [i].x, this.data [i].y);
						}
					}
				}
