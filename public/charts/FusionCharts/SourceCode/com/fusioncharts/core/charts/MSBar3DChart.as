	...
	*/
	private function calcVLinesPos ()
	{
		...
		for (i = 1; i <= numVLines; i ++)
		{
			...
			{
				//Set it's y position
				this.vLines [i].y = this.categories [this.vLines [i].index].y + (this.categories [this.vLines [i].index + 1].y - this.categories [this.vLines [i].index].y) / 2;
			} else 
			{
				//Invalidate it
	...
	private function calculatePoints ()
	{
		...
		if (this.params.xAxisName != "")
		{
			...
			if (this.params.rotateXAxisName)
			{
				...
				xAxisNameStyle.font = _embeddedFont;
				//Un-bold and un-italicize
				xAxisNameStyle.bold = false;
				xAxisNameStyle.italic = false;
		...
		//Next we check whether the legend is to be drawn
		...
		if (this.num > 2)
		{
			//If there are more than two blocks of bar on the chart, distribute them on the canvas equally .
			var blockSpace : Number = ((this.elements.canvasBg.h - 2 * this.params.canvasPadding) - (blockHeight * this.num)) / (this.num - 1);
		} else 
		{
			//If it's only 1 or 2 block of bars, we center align them instead of equal distribution.
			var blockSpace : Number = ((this.elements.canvasBg.h - 2 * this.params.canvasPadding) - (blockHeight * this.num)) / (this.num + 1);
		}
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.num > 2)
			{
				this.categories [i].y = this.elements.canvasBg.y + this.params.canvasPadding + (blockSpace * (i - 1)) + (blockHeight * (i - 0.5)) + this.config.shifts.yShift;
			}else
			{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.num > 2)
				{
					this.dataset [j].data [i].y = this.elements.canvasBg.y + this.params.canvasPadding + (blockSpace * (i - 1)) + (blockHeight * (i - 1)) + (oBarHeight * (j - 0.5)) + this.config.shifts.yShift;
				}else
				{
	...
	private function drawHeaders ()
	{
		...
		if (this.params.xAxisName != "")
		{
			...
			if (this.params.rotateXAxisName)
			{
				...
				xAxisNameStyleObj.font = _embeddedFont;
				//Set bold/italic to false.
				xAxisNameStyleObj.bold = false;
				xAxisNameStyleObj.italic = false;
				var xAxisNameObj : Object = createText (false, this.params.xAxisName, this.cMC, this.dm.getDepth ("XAXISNAME") , this.params.chartLeftMargin, this.elements.canvasBase.y + (this.elements.canvasBase.h / 2) , 90, xAxisNameStyleObj, false, 0, 0);
				//Adjust x for 90 rotation - as 90 is not automatically handled by createText
				xAxisNameObj.tf._x += xAxisNameObj.tf._width;
				xAxisNameObj.tf._y -= xAxisNameObj.tf._height/2;
				//Apply animation
				if (this.params.animation)
				{
		...
		for (i = this.num; i >= 1; i --)
		{
			for (j = this.numDS; j >=1 ; j--)
			{
				...
				if (this.dataset [j].data [i].isDefined)
				{
					...
					//Create bar instance
					barIns = new Bar3D (barMC, this.dataset [j].data [i].x, this.dataset [j].data [i].y, this.dataset [j].data [i].w, this.config.shifts.hFront,  this.config.shifts.xShift, this.config.shifts.yShift, this.dataset [j].data [i].color, this.params.showPlotBorder, this.params.plotBorderColor, this.params.plotBorderAlpha);
					//Draw the bar
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined && this.dataset [i].data [j].showValue)
				{
					...
					if (( ! this.params.placeValuesInside) && (this.dataset [i].data [j].w < 0) && ((xPos - valueObj.height) <= this.elements.canvasBg.x))
					{
