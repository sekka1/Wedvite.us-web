...
class com.fusioncharts.core.charts.LogMSColumn2DChart extends LogYAxisChart 
{
	...
	*/
	function LogMSColumn2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Logarithmic Axis Multi Series 2D Column Chart", Logger.LEVEL.INFO);
	...
	*/
	private function returnDataAsObject (value : Number, color : String, alpha : String, ratio : String, toolText : String, link : String, showValue : Number, isDashed : Boolean) : Object 
	{
		//Create a container
		...
		dataObj.isDefined = ((dataObj.alpha [0] == 0) || isNaN (value) || value == 0) ? false : true;
		//Absolute value - as logarithmic charts cannot plot negative values.
		dataObj.value = Math.abs (value);
		//Other parameters
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								//Now, get value.
								var dataValue : Number = this.getSetValue (atts ["value"]);
								var setValue : Number = (dataValue != 0) ? dataValue : undefined;
								...
								this.dataset [this.numDS].data [setCount] = this.returnDataAsObject (setValue, setColor, setAlpha, setRatio, setToolText, setLink, setShowValue, setDashed);
							}
						}
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		// ---------------------- LOGARITHMIC --------------------------//
		this.params.invertYAxis = toBoolean (getFN (atts ["invertyaxis"] , 0));
		//Base of the chart.
		this.params.base = getFN (atts ["base"] , 10);
		//Base cannot be <0 and cannot be 1
		if (this.params.base < 0 || this.params.base == 1)
		{
			//Set default to 10
			this.params.base = 10;
		}
		...
		// --------------------- CONFIGURATION ------------------------- //
		var yMin : Number = Math.abs (Number (atts ["yaxisminvalue"]));
		var yMax : Number = Math.abs (Number (atts ["yaxismaxvalue"]));
		this.params.yAxisMinValue = (yMin != 0) ?yMin : undefined;
		this.params.yAxisMaxValue = (yMax != 0) ?yMax : undefined;
		...
		this.params.showMinorDivLineValues = toBoolean (getFN (atts ["showminordivlinevalues"] , 0));
		//Show column shadows
		...
		this.params.divLineThickness = getFN (atts ["divlinethickness"] , 2);
		...
		//Minor div lines - between any two major div lines
		//Default value NOT needed for num minor div lines
		this.params.numMinorDivLines = atts ["numminordivlines"];
		this.params.minorDivLineColor = formatColor (getFV (atts ["minordivlinecolor"] , this.params.divLineColor));
		this.params.minorDivLineThickness = getFN (atts ["minordivlinethickness"] , 1);
		this.params.minorDivLineAlpha = getFN (atts ["minordivlinealpha"] , this.params.divLineAlpha / 2);
	...
	*/
	...
	{
		this.getAxisLimits (this.getMaxDataValue () , this.getMinDataValue ());
	}
	/**
	...
	*/
	private function calculatePoints ()
	{
		...
		{
			this.config.basePlanePos = this.getAxisPosition (this.config.yMin, null, null, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
		} 
		else
		{
			this.config.basePlanePos = this.getAxisPosition (this.config.yMax, null, null, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
		}
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				dataEndY = this.getAxisPosition (this.dataset [j].data [i].value, null, null, this.elements.canvas.y, this.elements.canvas.toY, true, 0);
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined)
				{
					...
					if (this.params.useRoundEdges){
						var colIns:RoundColumn2D = new RoundColumn2D(colMC, this.dataset [i].data [j].w-1, this.dataset [i].data [j].h, this.roundEdgeRadius, this.dataset [i].data [j].color[0].toString(16), this.params.plotBorderThickness, this.params.plotBorderAlpha);
					}else{	
						//Create column instance
						var colIns : Column2D =  new Column2D (colMC, this.dataset [i].data [j].w, this.dataset [i].data [j].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.dataset [i].data [j].color, this.dataset [i].data [j].alpha, this.dataset [i].data [j].ratio, this.params.plotFillAngle, false, this.dataset [i].data [j].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
					}
					//Draw the column
					colIns.draw ();
