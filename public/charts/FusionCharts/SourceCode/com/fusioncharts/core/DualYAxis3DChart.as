 /**
* @class DualYAxis3DChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
* DualYAxis3DChart extends DualYAxisChart class to encapsulate
* functionalities of a 3D chart with dual axes. All such
...
class com.fusioncharts.core.DualYAxis3DChart extends DualYAxisChart 
{
	//Instance variables
	...
	* constructor.
	*/
	function DualYAxis3DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
	...
	private function calcTrendLinePos ()
	{
		...
		for (i = 0; i <= this.numTrendLines; i ++)
		{
			...
			if (this.trendLines [i].isValid == true)
			{
				...
				this.trendLines [i].y = this.getAxisPosition (this.trendLines [i].startValue, (this.trendLines [i].parentYAxis == "P") ?this.config.PYMax : this.config.SYMax, (this.trendLines [i].parentYAxis == "P") ?this.config.PYMin : this.config.SYMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
				...
				if (this.trendLines [i].startValue != this.trendLines [i].endValue)
				{
					...
					this.trendLines [i].toY = this.getAxisPosition (this.trendLines [i].endValue, (this.trendLines [i].parentYAxis == "P") ?this.config.PYMax : this.config.SYMax, (this.trendLines [i].parentYAxis == "P") ?this.config.PYMin : this.config.SYMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
	...
	*/
	private function drawDivLines () : Void 
	{
		...
		for (i = 0; i < this.divLines.length; i ++)
		{
			...
			if (this.divLines [i].value == 0 && this.divLines [i].value != this.config.PYMin)
			{
			...
			else if ((i == 0) || (i == this.divLines.length - 1))
			{
				...
				if (this.params.showLimits && this.divLines [i].showValue)
				{
					...
					yPos = this.getAxisPosition (this.divLines [i].value, this.config.PYMax, this.config.PYMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
		...
		for (i = 0; i < this.sDivLines.length; i ++)
		{
			...
			if (showSDivValue)
			{
				...
				depth ++;
				//Get y position
				yPos = this.getAxisPosition (this.sDivLines [i].value, this.config.SYMax, this.config.SYMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
				//Create the text
	...
	*/
	private function drawZeroPlane (i) : Void
	{
		if (this.params.showZeroPlane)
		{
			...
			//Get y position
			var yPos : Number = this.getAxisPosition (0, this.config.PYMax, this.config.PYMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
			//Render the line
