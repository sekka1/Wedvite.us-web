...
*
* GridColumn3D extends the movie clip class and represents a single
* 3D Column to be drawn on the grid chart. It takes in the required
* parameters (dimensions) and draws a 3D column for grid.
*/
//Import required classes
import flash.geom.Rectangle;
import com.fusioncharts.extensions.ColorExt;
class com.fusioncharts.core.chartobjects.GridColumn3D extends MovieClip {
	//Instance variables
	...
	//Constructor method
	function GridColumn3D(target:MovieClip, x:Number, y:Number, wFront:Number, height:Number, xShift:Number, yShift:Number, color:String, showBorder:Boolean, borderColor:String, borderAlpha:Number) {
	...
	public function draw(use3DLighting:Boolean):Void {
		...
		if (height>=0) {
			...
			if (use3DLighting) {
				...
				var matrix:Object = {matrixType:"box", w:wFront, h:height, x:0, y:-height, r:90*(Math.PI/180)};
			...
			if (use3DLighting) {
				...
				var matrix:Object = {matrixType:"box", w:wFront+xShift, h:-yShift, x:-wHalf, y:-height, r:45*(Math.PI/180)};
				this.mc.beginGradientFill("linear", colors, alphas, ratios, matrix);
			} else {
			...
			if (use3DLighting) {
				...
				var matrix:Object = {matrixType:"box", w:wFront, h:height, x:wHalf, y:-height, r:90*(Math.PI/180)};
				this.mc.beginGradientFill("linear", colors, alphas, ratios, matrix);
			} else {
			...
			if (use3DLighting) {
				colors = [ColorExt.getLightColor(this.color, 0.55), ColorExt.getDarkColor(this.color, 0.65)];
				var matrix:Object = {matrixType:"box", w:wFront, h:-height, x:-wHalf-xShift, y:yShift, r:90*(Math.PI/180)};
				this.mc.beginGradientFill("linear", colors, alphas, ratios, matrix);
			} else {
				this.mc.beginFill(parseInt(this.color, 16), 100);
			}
			...
			if (use3DLighting) {
				//Gradient if use3DLighting
				colors = [ColorExt.getDarkColor(this.color, 0.85), ColorExt.getLightColor(this.color, 0.35)];
				var matrix:Object = {matrixType:"box", w:wFront+xShift, h:-yShift, x:-wHalf-xShift, y:yShift, r:45*(Math.PI/180)};
				this.mc.beginGradientFill("linear", colors, alphas, ratios, matrix);
			} else {
				//Solid fill
				this.mc.beginFill(ColorExt.getDarkColor(this.color, 0.75), 100);
			...
			if (use3DLighting) {
				colors = [ColorExt.getDarkColor(this.color, 0.75), ColorExt.getDarkColor(this.color, 0.35)];
				var matrix:Object = {matrixType:"box", w:wFront, h:-height, x:wHalf, y:0, r:90*(Math.PI/180)};
				this.mc.beginGradientFill("linear", colors, alphas, ratios, matrix);
			} else {
				this.mc.beginFill(ColorExt.getDarkColor(this.color, 0.6), 100);
			}
