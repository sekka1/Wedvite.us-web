...
function loadData ()
{
	...
	{		
		...
		if (_isSafeDataURL){
			...
			chart.log ("dataURL reset", "A colon character was found in dataURL, which can be potentially dangerous as it allows XSS attacks. Re-setting dataURL to Data.xml. If you're using absolute URLs (like http://domain.com/...) to provide dataURL, please convert it to relative path for increased security.", Logger.LEVEL.ERROR);
		}
...
function dataLoaded (success : Boolean) : Void 
{
	...
	if (success)
	{
		...
		//TODO: Debug this line. Playhead not going to DataLoad,FDataLoadDone when chart laoded with this sequence: 1. Normal data.xml, 2. Error data using setDataXML, 3. setDataURL to a valid XML
		gotoAndPlay ("DataLoad", "FDataLoadDone");
	} else 
	{
		//Delete the enter frame event
...
function filterXSSChars(strURL:String):String{
	//If we're working in online mode and a colon character was found in the URL.
	...
	if ((_isOnline==true) && (strURL.indexOf(":")!=-1 || strURL.indexOf("%3A")!=-1)){
